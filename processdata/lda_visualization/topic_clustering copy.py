...
# 将主题概率转换为向量
def convert_to_vector(all_probabilities_str, num_topics=6):
    if pd.isna(all_probabilities_str):
        return [0] * num_topics
    
    try:
        # 解析概率字符串为数值列表
        # 字符串格式类似 "0.1,0.2,0.05,0.3,0.25,0.1"（逗号分隔的浮点数）
...
# 使用多种降维方法并结合它们的结果
def reduce_dimensions(vectors, n_components=2, perplexity=30, random_state=42, n_neighbors=8, min_dist=1.0, spread=2.0, scale=2.0):
    # 添加微小的噪声以增加数据的可分性
    ...
    reducer = umap.UMAP(
        ...
        n_neighbors=n_neighbors,  # 使用更小的邻居数量，使点更分散
        min_dist=min_dist,        # 使用更大的最小距离，使点之间距离更大
        spread=spread,            # 使用更大的spread参数，使分布更均匀且分散
        random_state=random_state
    )
    umap_result = reducer.fit_transform(vectors_with_noise)
...
def create_interactive_plot(coords, labels, df, vectors, output_file='topic_clusters_interactive.png',
    ...
    if use_topic_colors:
        ...
        for i in range(len(vectors)):
            ...
            # 计算该点对背景的颜色贡献
            point_color = get_topic_color(probabilities, color_palette)
            background_colors += weights * point_color
    else:
        # 使用聚类标签进行背景染色
        background_colors = np.zeros((len(grid_points), 4))
    ...
    # 归一化背景色
    background_sum = np.sum(background_colors, axis=1)[:, np.newaxis]
    background_sum[background_sum == 0] = 1  # 避免除以0
    background_colors = background_colors / background_sum
    ...
    if use_topic_colors:
        ...
        for topic_idx in range(num_topics):
            ...
            if np.sum(mask) > 0:  # 如果有点属于该主题
                ...
                # 应用抖动
                jittered_points = topic_points + np.random.normal(0, jitter, topic_points.shape)
    ...
    if -1 in labels:
        ...
        if len(noise_points) > 0:
            # 添加随机偏移
            jittered_noise = noise_points + np.random.normal(0, jitter, noise_points.shape)
...
def parse_args():
    ...
    # UMAP参数
    parser.add_argument('--n_neighbors', type=int, default=3, help='UMAP的邻居数量，较小的值使聚类更分散')
    parser.add_argument('--min_dist', type=float, default=2.0, help='UMAP的最小距离，较大的值使点之间距离更大')
    parser.add_argument('--spread', type=float, default=4.0, help='UMAP的spread参数，较大的值使分布更分散')
    parser.add_argument('--scale', type=float, default=3.0, help='坐标缩放因子，较大的值使整体分布更大')
    
    # HDBSCAN参数
    parser.add_argument('--min_cluster_size', type=int, default=50, help='HDBSCAN最小聚类大小')
    parser.add_argument('--min_samples', type=int, default=10, help='HDBSCAN最小样本数')
    parser.add_argument('--cluster_selection_epsilon', type=float, default=0.5, help='HDBSCAN聚类选择epsilon')
    
    # 传统聚类参数
    parser.add_argument('--n_clusters', type=int, default=6, help='聚类数量(当使用K-means时)')
    ...
    parser.add_argument('--jitter', type=float, default=0.03, help='点的抖动程度，用于减少重叠')
    ...
    # 输入文件
    parser.add_argument('--input', type=str, default='topics_probabilities.csv', help='输入CSV文件路径')
...
def main():
    ...
    try:
        ...
        if not found:
            print("错误: 无法找到topics_probabilities.csv文件。请确保文件存在并提供正确路径。")
            print("可以使用 --input 参数指定文件路径，例如: --input=./topics_probabilities.csv")
            return
    ...
    for prob_str in topic_df['allProbabilities']:
        vector = convert_to_vector(prob_str)
